...
import { SQSQueue } from "webda-aws";
...
export default class CloudTrailService extends AWSServiceMixIn(SQSQueue) {
  ...
  async init(): Promise<void> {
    ...
    this._validatorService = <ValidatorService>(
      this.getService("IvoryShield/ValidatorService")
    );
    this._cloudtrailSetup = <CloudTrailSetup>(
      this.getService("IvoryShield/CloudTrailSetup")
    );
  ...
  test(evtFile) {
    ...
    return this.processTrailEvent(
      JSON.parse(fs.readFileSync(evtFile).toString())
    );
  }
  ...
  async processTrailEvent(evt) {
    ...
    try {
      return this._validatorService.handleEvent(
        aws,
        evt,
        evt.recipientAccountId
      );
    } catch (err) {
  ...
  processTrailQueue(s3evt) {
    ...
    s3evt.Records.forEach(evt => {
      if (evt.eventName === "ObjectCreated:Put") {
        ...
        promises.push(
          this.processTrailLog(evt.s3.bucket.name, evt.s3.object.key)
        );
      }
    });
  ...
  private initES() {
    if (this._elkSetup) {
      ...
      this._params.elasticsearchIndex =
        this._params.elasticsearchIndex || "logstash-";
    }
  }
  ...
  async saveEvent(evt) {
    ...
    esData.index =
      this._params.elasticsearchIndex +
      evt.eventTime.substring(0, 10).replace(new RegExp("-", "g"), ".");
